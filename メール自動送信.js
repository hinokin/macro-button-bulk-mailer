...
function mailauto(listnum) {
  //ドキュメントから件名と本文を取得　//①
{
  var doc_video = DocumentApp.openById('1JYwOK_rNEhx0rAI7YF2CooVSJBSbBk-UcKk54PrDkgs');//1.★使用するドキュメントシートのIDを記入（動画・映像）
  var subject_video = doc_video.getName();
  var text_video = doc_video.getBody().getText();

  var doc_eat = DocumentApp.openById('1_gcWpnFBwktNnvfw5Vbmw_Uiw3FLFQT7gSAfDJBSjrE');//2.★使用するドキュメントシートのIDを記入（飲食）
  var subject_eat = doc_eat.getName();
  var text_eat = doc_eat.getBody().getText();

  var doc_teachi = DocumentApp.openById('1530UeYoV603lHAXEfpfczKZwMDOFBZC0j_PMqQ9WiPc');//3.★使用するドキュメントシートのIDを記入（教育）
  var subject_teachi = doc_teachi.getName();
  var text_teachi = doc_teachi.getBody().getText();

  var doc_ad = DocumentApp.openById('1wxxysbZ2KT0FKqgq7Xz5yJJveeeWrbPBnx8vmBR3Am0');//4.★使用するドキュメントシートのIDを記入（広告）
  var subject_ad = doc_ad.getName();
  var text_ad = doc_ad.getBody().getText();

  var doc_ent = DocumentApp.openById('1GiwwoCaNBYmMMjo_KLjVr0Pgjb2v5ObnFt1_SuOF_Fs');//5.★使用するドキュメントシートのIDを記入（エンタメ）
  ...
  var doc_video2022 = DocumentApp.openById('1rbA3euh-3bIvt9NAZUmLqqLDVO-LbLNU3al-U6o--oM');//数字.★使用するドキュメントシートのIDを記入（新・動画）2022年は【15秒のCM動画】を制作しませんか？
  var subject_video2022 = doc_video2022.getName();
  var text_video2022 = doc_video2022.getBody().getText();

  var doc_YTso = DocumentApp.openById('1RpdmW0MOl0UQk3surxGnVXSLk6D9O5npeAOksJR6vaw');//16.★使用するドキュメントシートのIDを記入（YT_so）YouTubeチャンネル診断のご相談
  var subject_YTso = doc_YTso.getName();
  var text_YTso = doc_YTso.getBody().getText();

  var doc_interv = DocumentApp.openById('1fdpBqLobbLmjj361seswq1PGp22m_VXSC3U2cuRgxK8');//17.★使用するドキュメントシートのIDを記入（インタビュー取材）
  var subject_interv = doc_interv.getName();
  var text_interv = doc_interv.getBody().getText();

  var doc_ChatGPT = DocumentApp.openById('1Xk4LA6Qfg2HULU97-RqEunkOL5R9CgVMwRW8SNACCkw');//数字.★使用するドキュメントシートのIDを記入（新規業界名(変更箇所)）
