...
function mail(listnum){
  ...
  //ドキュメントから件名と本文を取得　//①
{
  var doc_video = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//1.★使用するドキュメントシートのIDを記入（動画・映像）動画制作の協力の相談
  var subject_video = doc_video.getName();
  var text_video = doc_video.getBody().getText();

  var doc_eat = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//2.★使用するドキュメントシートのIDを記入（飲食）飲食系の動画制作のご相談
  var subject_eat = doc_eat.getName();
  var text_eat = doc_eat.getBody().getText();

  var doc_teachi = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//3.★使用するドキュメントシートのIDを記入（教育）教育系動画制作のご相談
  var subject_teachi = doc_teachi.getName();
  var text_teachi = doc_teachi.getBody().getText();

  var doc_ad = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//4.★使用するドキュメントシートのIDを記入（広告）広告系の動画制作のご相談
  var subject_ad = doc_ad.getName();
  var text_ad = doc_ad.getBody().getText();

  var doc_ent = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//5.★使用するドキュメントシートのIDを記入（エンタメ）エンタメ系の動画制作のご相談
  ...
  var doc_all = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//14.★使用するドキュメントシートのIDを記入（オール）動画制作の協力の相談
  var subject_all = doc_all.getName();
  var text_all = doc_all.getBody().getText();

  var doc_video2022 = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//15.★使用するドキュメントシートのIDを記入（新・動画）2022年は【15秒のCM動画】を制作しませんか？
  var subject_video2022 = doc_video2022.getName();
  var text_video2022 = doc_video2022.getBody().getText();

  var doc_YTso = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//16.★使用するドキュメントシートのIDを記入（YouTubeチャンネル診断のご相談）
  var subject_YTso = doc_YTso.getName();
  var text_YTso = doc_YTso.getBody().getText();
  
  var doc_interv = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//17.★使用するドキュメントシートのIDを記入（インタビュー取材のご相談）
  var subject_interv = doc_interv.getName();
  var text_interv = doc_interv.getBody().getText();

  var doc_ChatGPT = DocumentApp.openById('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');//数字.★使用するドキュメントシートのIDを記入（ChatGPT）
